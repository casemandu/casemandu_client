const getAllOptions = async () => {
  try {
    ...
    const apiUrl = `${process.env.NEXT_PUBLIC_API_URL}/api/options`
    ...
    try {
      ...
      if (fetchError.name === 'TypeError' && fetchError.message.includes('Failed to fetch')) {
        console.warn('CORS error or network issue fetching options. This may be due to API CORS configuration.', {
          url: apiUrl,
      ...
      if (fetchError.name === 'AbortError') {
        console.error('Options request timed out after 10 seconds', { url: apiUrl })
      } else {
        console.error('Network error fetching options:', fetchError, { url: apiUrl })
      }
      return []
    }
